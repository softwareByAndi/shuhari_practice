...
export function Numpad({
  ...
  const renderButton = (label: string, onClick: () => void, className = '') => (
    <button
      key={label}
      onClick={onClick}
      className={`flex-grow text-xl xxs:text-2xl xs:text-3xl sm:text-4xl font-semibold md:font-bold  rounded-lg transition-all
  ...
  return (
    ...
    <div className="flex-grow grid grid-cols-3 gap-1 xxs:gap-2 xs:gap-3 h-full w-full">
      {/* Numbers 1-9 */}
